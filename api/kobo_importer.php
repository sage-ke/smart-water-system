...
try {
    ...
    $kobo_sync_log = $pdo->query("SELECT * FROM kobo_sync_log
        ORDER BY synced_at DESC LIMIT 5")->fetchAll();
} catch(PDOException $e) {}

// KoBo settings — check if configured
$kobo_enabled = false;
$kobo_configured = false;
try {
    $ks = $pdo->query("SELECT setting_key, setting_val FROM system_settings
        WHERE setting_key IN ('kobo_enabled','kobo_api_token','kobo_asset_uid',
        'kobo_last_sync')")->fetchAll(PDO::FETCH_KEY_PAIR);
    $kobo_enabled    = ($ks['kobo_enabled'] ?? '0') === '1';
    $kobo_configured = !empty($ks['kobo_api_token']) && !empty($ks['kobo_asset_uid']);
    $kobo_last_sync  = $ks['kobo_last_sync'] ?? '';
} catch(PDOException $e) {}
...
<!-- CSV Upload -->
<!-- KoBo Live Sync Panel -->
<div class="sec">📡 KoBo Live Sync</div>
<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;
     padding:1.25rem;margin-bottom:1.5rem">

    <?php if(!$kobo_configured): ?>
    <!-- Not configured -->
    <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap">
        <div style="font-size:2rem">📡</div>
        <div>
            <div style="font-weight:700;margin-bottom:4px">KoBo Live Sync not configured</div>
            <div style="font-size:.82rem;color:var(--muted);line-height:1.6">
                Connect to KoBo Toolbox to automatically pull field officer reports.<br>
                Field officers fill forms on their phones (works offline) and submissions sync here instantly.
            </div>
        </div>
        <a href="settings.php#kobo" style="margin-left:auto;padding:9px 20px;
           background:linear-gradient(135deg,#0ea5e9,#06b6d4);border-radius:8px;
           color:#fff;font-weight:700;font-size:.85rem;text-decoration:none;white-space:nowrap">
            ⚙️ Configure KoBo →
        </a>
    </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;
        ...
        <div>
            <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:6px">
                <span style="width:8px;height:8px;border-radius:50%;
                      background:<?php echo $kobo_enabled?'#34d399':'#f87171'; ?>;
                      display:inline-block"></span>
                <span style="font-weight:700">
                    KoBo Sync <?php echo $kobo_enabled?'<span style="color:#34d399">Active</span>':'<span style="color:#f87171">Disabled</span>'; ?>
                </span>
                <a href="settings.php" style="font-size:.72rem;color:var(--muted)">Edit settings ↗</a>
            </div>
            <?php if($kobo_last_sync): ?>
            <div style="font-size:.78rem;color:var(--muted)">
                🕐 Last sync: <?php echo htmlspecialchars($kobo_last_sync); ?>
            </div>
            <?php endif; ?>
        </div>
        <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap">
            <span id="koboStatus" style="font-size:.82rem;color:var(--muted)"></span>
            <button onclick="runKoboSync()"
                id="btnSync"
                style="padding:9px 22px;background:linear-gradient(135deg,#0ea5e9,#06b6d4);
    ...
    <!-- Sync log table -->
    <?php if(!empty($kobo_sync_log)): ?>
    <div style="font-size:.72rem;color:var(--muted);margin-bottom:.5rem;font-weight:600;
         text-transform:uppercase;letter-spacing:.06em">Recent Syncs</div>
    <table style="width:100%;border-collapse:collapse;font-size:.78rem">
        <thead>
            <tr style="color:var(--muted);font-size:.68rem;text-transform:uppercase">
                <th style="text-align:left;padding:5px 8px">Time</th>
                <th style="text-align:center;padding:5px 8px">Fetched</th>
                <th style="text-align:center;padding:5px 8px">Imported</th>
                <th style="text-align:center;padding:5px 8px">Skipped</th>
                <th style="text-align:left;padding:5px 8px">Status</th>
            </tr>
        </thead>
        <tbody>
        <?php foreach($kobo_sync_log as $log): ?>
        <tr style="border-top:1px solid rgba(30,58,95,.3)">
            <td style="padding:6px 8px;color:var(--muted)">
                <?php echo date('d M H:i', strtotime($log['synced_at'])); ?>
            </td>
            <td style="padding:6px 8px;text-align:center"><?php echo $log['total_fetched']; ?></td>
            <td style="padding:6px 8px;text-align:center;color:#34d399;font-weight:700">
                +<?php echo $log['imported']; ?>
            </td>
            <td style="padding:6px 8px;text-align:center;color:var(--muted)"><?php echo $log['skipped']; ?></td>
            <td style="padding:6px 8px">
                <span style="padding:2px 8px;border-radius:5px;font-size:.65rem;font-weight:700;
                      background:<?php echo $log['status']==='success'?'rgba(52,211,153,.15)':'rgba(251,191,36,.15)'; ?>;
                      color:<?php echo $log['status']==='success'?'#34d399':'#fbbf24'; ?>">
    ...
    <?php else: ?>
    <div style="color:var(--muted);font-size:.82rem">No syncs yet — click Sync Now to pull submissions from KoBo.</div>
    <?php endif; ?>
    <?php endif; ?>
</div>
...
<table class="tbl">
  ...
  <tr>
    ...
    <td style="font-size:.7rem">
        <?php
        $src_colors = ['kobo'=>'#34d399','csv_upload'=>'#0ea5e9',
                       'web_form'=>'#a78bfa','api'=>'#fbbf24'];
        $src_col = $src_colors[$c['source']] ?? '#7a9bba';
        ?>
        <span style="color:<?=$src_col?>"><?=$c['source']?></span>
    </td>
...
// ── KoBo Live Sync ─────────────────────────────────────────────
async function runKoboSync() {
    const btn    = document.getElementById('btnSync');
    const status = document.getElementById('koboStatus');
    if (!btn) return;
    ...
    try {
        const res  = await fetch('api/kobo_sync.php', {
        ...
        if (data.status === 'success') {
            ...
            // Reload after 2s to show new complaints + updated log
            setTimeout(() => window.location.reload(), 2000);
        } else if (data.status === 'disabled') {
            status.style.color = '#fbbf24';
            status.textContent = '⚠️ Sync disabled — enable in Settings';
        } else {
        ...
        status.style.color = '#f87171';
        status.textContent = '❌ Request failed — is XAMPP running?';
    }
